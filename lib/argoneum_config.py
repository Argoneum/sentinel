...
class ArgoneumConfig():

    @classmethod
    def slurp_config_file(self, filename):
        # read argoneum.conf config but skip commented lines
        f = io.open(filename)
        lines = []
        for line in f:
            if re.match('^\s*#', line):
        ...
        # data is argoneum.conf without commented lines
        data = ''.join(lines)
    ...
    def get_rpc_creds(self, data, network='mainnet'):
        # get rpc info from argoneum.conf
        match = re.findall(r'rpc(user|password|port)=(.*?)$', data, re.MULTILINE)

        # python >= 2.7
        creds = {key: value for (key, value) in match}

        # standard Argoneum defaults...
        default_port = 9899 if (network == 'mainnet') else 10899

        # use default port for network if not specified in argoneum.conf
        if not ('port' in creds):
            creds[u'port'] = default_port

        # convert to an int if taken from argoneum.conf
