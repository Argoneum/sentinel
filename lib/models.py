...
class GovernanceObject(BaseModel):
    ...
    @classmethod
    def sync(self, argoneumd):
        golist = argoneumd.rpc_command('gobject', 'list')
        ...
        for item in golist.values():
            try:
                (go, subobj) = self.import_gobject_from_argoneumd(argoneumd, item)
            except Exception as e:
    ...
    @classmethod
    def import_gobject_from_argoneumd(self, argoneumd, rec):
        import decimal
        import argoneumlib
        import binascii
        import gobject_json
        ...
        # exclude any invalid model data from argoneumd...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from argoneumd, with every run
        ...
        # get/create, then sync payment amounts, etc. from argoneumd - Argoneumd is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(argoneumd)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from argoneumd! %s" % e)
            govobj.vote_delete(argoneumd)
            return (govobj, None)
    ...
    def vote_delete(self, argoneumd):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(argoneumd, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, argoneumd, signal, outcome):
        import argoneumlib
        ...
        output = argoneumd.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = argoneumlib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(argoneumd, signal)

    def sync_network_vote(self, argoneumd, signal):
        printdbg('\tSyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = argoneumd.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    govobj_type = ARGONEUMD_GOVOBJ_TYPES['proposal']
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 argoneum addr, non-multisig
            if not argoneumlib.is_valid_argoneum_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid Argoneum address for network [%s], returning False" % (self.payment_address, config.network))
                return False
            ...
            # Argoneum Core restricts proposals to 512 bytes max
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (argoneumlib.blocks_to_seconds(superblockcycle) / 2) +
...
class Superblock(BaseModel, GovernanceClass):
    ...
    govobj_type = ARGONEUMD_GOVOBJ_TYPES['superblock']
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not argoneumlib.is_valid_argoneum_address(addr, config.network):
    ...
    def hash(self):
        import argoneumlib
        return argoneumlib.hashit(self.serialise())
