...
class GovernanceClass(object):
    ...
    def vote(self, argoneumd, signal, outcome):
        return self.go.vote(argoneumd, signal, outcome)

    # pass thru to GovernanceObject#voted_on
    def voted_on(self, **kwargs):
        return self.go.voted_on(**kwargs)

    def vote_validity(self, argoneumd):
        if self.is_valid():
            printdbg("Voting valid! %s: %d" % (self.__class__.__name__, self.id))
            self.vote(argoneumd, models.VoteSignals.valid, models.VoteOutcomes.yes)
        else:
            printdbg("Voting INVALID! %s: %d" % (self.__class__.__name__, self.id))
            self.vote(argoneumd, models.VoteSignals.valid, models.VoteOutcomes.no)

    def get_submit_command(self):
        import argoneumlib
        obj_data = argoneumlib.SHIM_serialise_for_argoneumd(self.serialise())
    ...
    def argoneumd_serialise(self):
        import argoneumlib
        return argoneumlib.SHIM_serialise_for_argoneumd(self.serialise())
