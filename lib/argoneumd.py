...
class ArgoneumDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some argoneumd methods
    ...
    @classmethod
    def from_argoneum_conf(self, argoneum_dot_conf):
        from argoneum_config import ArgoneumConfig
        config_text = ArgoneumConfig.slurp_config_file(argoneum_dot_conf)
        creds = ArgoneumConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = argoneumlib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = argoneumlib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = argoneumlib.blocks_to_seconds(diff)
    ...
    def has_sentinel_ping(self):
        ...
        return (getinfo['protocolversion'] >= config.min_argoneumd_proto_version_with_sentinel_ping)
