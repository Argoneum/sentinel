...
sentinel_cfg = ArgoneumConfig.tokenize(sentinel_config_file)
sentinel_version = "1.2.0"
min_argoneumd_proto_version_with_sentinel_ping = 70207


def get_argoneum_conf():
    if sys.platform == 'win32':
        argoneum_conf = os.path.join(os.getenv('APPDATA'), "Argoneum/argoneum.conf")
    else:
        home = os.environ.get('HOME')

        argoneum_conf = os.path.join(home, ".argoneum/argoneum.conf")
        if sys.platform == 'darwin':
            argoneum_conf = os.path.join(home, "Library/Application Support/Argoneum/argoneum.conf")

    argoneum_conf = sentinel_cfg.get('argoneum_conf', argoneum_conf)

    return argoneum_conf
...
argoneum_conf = get_argoneum_conf()
network = get_network()
