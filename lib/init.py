...
def has_argoneum_conf():
    import config
    import io

    valid_argoneum_conf = False

    # ensure argoneum_conf exists & readable
    #
    # if not, print a message stating that Argoneum Core must be installed and
    # configured, including JSONRPC access in argoneum.conf
    try:
        f = io.open(config.argoneum_conf)
        valid_argoneum_conf = True
    except IOError as e:
        print(e)

    return valid_argoneum_conf
...
def main():
    ...
    if not has_argoneum_conf():
        print("Argoneum Core must be installed and configured, including JSONRPC access in argoneum.conf")
        sys.exit(1)
