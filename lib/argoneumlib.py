...
def is_valid_argoneum_address(address, network='mainnet'):
    ...
    argoneum_version = 140 if network == 'testnet' else 76
    ...
    try:
        ...
        # rescue from exception, not a valid Argoneum address
        return False

    if (address_version != argoneum_version):
        return False
...
def create_superblock(proposals, event_block_height, budget_max, sb_epoch_time, maxgovobjdatasize):
    ...
    for proposal in proposals:
        ...
        data_size = len(sb_temp.argoneumd_serialise())
...
def SHIM_serialise_for_argoneumd(sentinel_hex):
    ...
    # superblock => "trigger" in argoneumd
    if govtype_string == 'superblock':
        govtype_string = 'trigger'

    # argoneumd expects an array (will be deprecated)
    obj = [(govtype_string, obj,)]

    # re-pack
    argoneumd_hex = serialise(obj)
    return argoneumd_hex
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('argoneum.conf')
